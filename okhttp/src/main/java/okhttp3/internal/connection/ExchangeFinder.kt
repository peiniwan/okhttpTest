...
class ExchangeFinder(
  ...
  fun find(
    ...
    try {
      val resultConnection = findHealthyConnection(//找到健康的链接
      ...
      return resultConnection.newCodec(client, chain)//编解码器，发报文返回报文
   ...
   */
  @Throws(IOException::class)
  private fun findConnection(//找到合适的连接
    connectTimeout: Int,
    readTimeout: Int,
    writeTimeout: Int,
    pingIntervalMillis: Int,
    ...
    val toClose: Socket?
    synchronized(connectionPool) {
      if (call.isCanceled()) throw IOException("Canceled") //取消了结束
      ...
      toClose = if (callConnection != null && (callConnection.noNewExchanges ||
              !sameHostAndPort(callConnection.route().address.url))) {  //有可用连接，但是不合适新请求，释放连接：超时了，
    ...
    synchronized(connectionPool) {
      ...
      if (newRouteSelection) {
        ...
        routes = routeSelection!!.routes  //代理，选择新的路由，再次尝试从连接池获取
    ...
    // Do TCP + TLS handshakes. This is a blocking operation. 都没拿到，就自己去创建，5次，有一次找到就返回了
    result!!.connect(
        connectTimeout,
        readTimeout,
        writeTimeout,
    ...
    synchronized(connectionPool) {
      ...
      if (connectionPool.callAcquirePooledConnection(address, call, routes, true)) {//多路的时候，共享连接，拿池里的连接
        // We lost the race! Close the connection we created and return the pooled connection.
        result!!.noNewExchanges = true //allin.com/1 /2 /3 省资源
