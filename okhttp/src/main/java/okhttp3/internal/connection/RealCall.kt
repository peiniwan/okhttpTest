...
class RealCall(
  ...
  internal fun getResponseWithInterceptorChain(): Response {
    ...
    interceptors += client.interceptors //俩个自定义拦截器,原始数据，没经过BridgeInterceptor，加了一些头，一般改东西在这
    interceptors += RetryAndFollowUpInterceptor(client)
    interceptors += BridgeInterceptor(client.cookieJar)
    interceptors += CacheInterceptor(client.cache)
    interceptors += ConnectInterceptor  //连接，主要负责和服务器建立连接
    if (!forWebSocket) {
      interceptors += client.networkInterceptors //一般做网络调试自定义，比如addNetworkInterceptors  stretho
    }
    interceptors += CallServerInterceptor(forWebSocket) //数据流拦截器，io操作，报文封装和解析。
  ...
  fun enterNetworkInterceptorExchange(request: Request, newExchangeFinder: Boolean) {
    ...
    if (newExchangeFinder) {
      this.exchangeFinder = ExchangeFinder(
          connectionPool,
          createAddress(request.url),//创建一个地址，后面用
          this,
          eventListener
      )
    }
