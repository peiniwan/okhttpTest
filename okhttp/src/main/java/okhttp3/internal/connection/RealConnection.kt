...
class RealConnection(//真正的请求
  ...
  private fun connectTunnel(
    ...
    for (i in 0 until MAX_TUNNEL_ATTEMPTS) {//首先先做一个HTTP连接，然后去做ssl连接
  ...
  private fun connectSocket(
    ...
    try {
      source = rawSocket.source().buffer()//返回数据
  ...
  private fun establishProtocol(
    ...
    connectTls(connectionSpecSelector)//tls连接
  ...
  private fun connectTls(connectionSpecSelector: ConnectionSpecSelector) {
    ...
    try {
      ...
      if (!address.hostnameVerifier!!.verify(address.url.host, sslSocketSession)) {//验证证书
  ...
  internal fun isEligible(address: Address, routes: List<Route>?): Boolean {
    ...
    if (calls.size >= allocationLimit || noNewExchanges) return false//请求数没有超限，HTTP1就一个请求

    // If the non-host fields of the address don't overlap, we're done.
    if (!this.route.address.equalsNonHost(address)) return false//IP一样

    // If the host exactly matches, we're done: this connection can carry the address.
    if (address.url.host == this.route().address.url.host) {//host一样
    ...
    if (routes == null || !routeMatchesAny(routes)) return false  //传了才往下走

    // 3. This connection's server certificate's must cover the new host.
    if (address.hostnameVerifier !== OkHostnameVerifier) return false
    if (!supportsUrl(address.url)) return false

    // 4. Certificate pinning must match the host. ssl也得一样
    try {
  ...
  internal fun newCodec(client: OkHttpClient, chain: RealInterceptorChain): ExchangeCodec {
    ...
    return if (http2Connection != null) {
      Http2ExchangeCodec(client, this, chain, http2Connection)//2就返回2的编解码，1就返回1的编解码器
    } else {
