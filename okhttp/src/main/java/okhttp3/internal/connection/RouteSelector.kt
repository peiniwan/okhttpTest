...
class RouteSelector(
  ...
  private fun resetNextInetSocketAddress(proxy: Proxy) {
    ...
    val socketPort: Int
    if (proxy.type() == Proxy.Type.DIRECT || proxy.type() == Proxy.Type.SOCKS) {//如果是直连就是直连
      ...
      socketHost = proxyAddress.socketHost  //else就是代理的地址
    ...
    if (proxy.type() == Proxy.Type.SOCKS) {
      ...
      val addresses = address.dns.lookup(socketHost)  //lookup解析IP地址
